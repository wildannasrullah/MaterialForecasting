    ...
    <div class='md-card'>
		...
		<div class='uk-margin' data-uk-grid-margin>
                            <div class='uk-width-medium-1-0'>
								...
								<form method='post' action='<?php echo "$aksi?n=forecasting-result&act=filter_result&c=$_GET[c]&ct=$_GET[ct]";?>'>
                                    <select id='select_demo_4' name='periode' class='md-input'>
										<?php
											echo "<option>------Available Period-------</option>";
											$qq = mysqli_query($conn, "select *from forecasting_ppic_totqty where CustomerCode='$_GET[c]'");
											while($b = mysqli_fetch_array($qq)){
												if($b[begda]=='01') $nameb = 'January';
												else if($b[begda]=='02') $nameb = 'February';
												else if($b[begda]=='03') $nameb = 'March';
												else if($b[begda]=='04') $nameb = 'April';
												else if($b[begda]=='05') $nameb = 'May';
												else if($b[begda]=='06') $nameb = 'June';
												else if($b[begda]=='07') $nameb = 'July';
												else if($b[begda]=='08') $nameb = 'August';
												else if($b[begda]=='09') $nameb = 'September';
												else if($b[begda]=='10') $nameb = 'October';
												else if($b[begda]=='11') $nameb = 'Nopember';
												else $nameb = 'December';
												
												if($b[endda]=='01') $namee = 'January';
												else if($b[endda]=='02') $namee = 'February';
												else if($b[endda]=='03') $namee = 'March';
												else if($b[endda]=='04') $namee = 'April';
												else if($b[endda]=='05') $namee = 'May';
												else if($b[endda]=='06') $namee = 'June';
												else if($b[endda]=='07') $namee = 'July';
												else if($b[endda]=='08') $namee = 'August';
												else if($b[endda]=='09') $namee = 'September';
												else if($b[endda]=='10') $namee = 'October';
												else if($b[endda]=='11') $namee = 'Nopember';
												else $namee = 'December';
												
												echo "<option value='$b[begda]|$b[endda]'>$nameb $b[year] - $namee $b[year]";
											}
										?>
									</select>
									<p align='right'><button type='submit' name='filter' class='md-btn md-btn-small md-btn-success md-btn-wave'>Process</button></p>
								</div>
                            </div>
                        </div>
			<div class='md-card uk-margin-medium-bottom'>
                <div class='md-card-content'>
					...
					<?php	
						...
						$result= mysqli_query($conn, "CALL TotalForecastPPICFix('$_GET[c]','$_GET[ct]','$_GET[b]','$_GET[e]');"); 
						/* if ($rest)
						...
						";
						 ...
						 //TOTAL FORECASTING
						 echo "
						<tr><td></td><td><b>TOTAL (PCS)</b></td>";
							for($h=2; $h<mysqli_num_fields($result); $h++)
							{ 
						 ...
						 echo"</tr>";
						  ...
						  echo "<tr><td></td><td><b>JUMLAH UP</b></td>";
						 ...
						 echo"</tr>";
						 //TOTAL KEBUTUHAN
						  echo "<tr><td></td><td><b>TOTAL KEBUTUHAN (REAM)</b></td>";
							for($h=2; $h<mysqli_num_fields($result); $h++)
							{ 
								if($ju[$h]==NULL){
									$kebutuhan[$h] = "";
								}else{
									$kebutuhan[$h] = round((($totfor[$h]/$ju[$h])/500),3);
								}
								echo "<td align=right><b>".$kebutuhan[$h]."</b></td>";
							} 
						 echo"</tr>";
						 //WAREHOUSE
						  echo "<tr><td bgcolor='#76ead7'>(REAM)</td><td bgcolor='#76ead7'><b>WAREHOUSE</b></td>";
						  $rest= mysqli_query($conne, "CALL WarehousePPIC('$_GET[c]','$_GET[ct]','$_GET[b]','$_GET[e]');");
						  while ($rw = mysqli_fetch_row($rest))
							{
								for($j2=2; $j2<mysqli_num_fields($rest); $j2++)
								{ 
									if($ju[$j2]==NULL){
										$warehouse[$j2]="";
									}
									else{
										$warehouse[$j2]=round((($rw[$j2])/500),3);
									}
									echo '<td align=right bgcolor=#76ead7><b>'.$warehouse[$j2].'</b></td>';
									$rim[$j2]=$rw[$j2]/500;
								}
							}
						 echo"</tr>";
						  echo "<tr><td bgcolor='#76ead7'>(PCS)</td><td bgcolor='#76ead7'></td>";
							for($h=2; $h<mysqli_num_fields($result); $h++)
							{ 
								if($ju[$h]==NULL){
										$warehouse[$h]="";
										...
										$warehouse[$h]=round((($rim[$h]*$ju[$h])*500),3);
								}
								echo "<td align=right bgcolor='#76ead7'><b>".$warehouse[$h]."</b></td>";
							} 
						 echo"</tr>";
						 //CEK PO Outstanding
						 $polist= mysqli_query($connec, "CALL OutstandingPO('$_GET[c]','$_GET[ct]','$_GET[b]','$_GET[e]');");
							while ($row = mysqli_fetch_row($polist))
							{	
								 echo "<tr>";
								 echo '<td></td><td align=right><font color="red"><b>'.$row[2].'</b></font></td>';
								for($j=2; $j<(mysqli_num_fields($polist)); $j++)
								{ 
									if($ju[$j]==NULL){
										$outstand[$j]="";
									}
									else{
											$outstand[$j]=round($row[$j],4);
									}
									echo '<td align=right><b>'.$outstand[$j].'</b></td>';
								...
								echo "</tr>";
								 echo "<tr>";
								 echo '<td></td><td align=right><font color="red"><b>&nbsp;</b></font></td>';
									for($i=2; $i<(mysqli_num_fields($polist)); $i++)
									{ 
										if($ju[$i]==NULL){
											$out[$i]="";
										}else{
											$out[$i]=round((($ju[$i]*$nilai[$i])*500),4);
										}
										$outstanding[$i] += (($ju[$i]*$nilai[$i])*500);
										echo '<td align=right><b>'.$out[$i].'</b></td>';
									}
								echo "</tr>";
							}
							...
							//MINUS PAPER
							 ...
							 //PCS
							 echo "
							 <tr><td bgcolor='#fbfd8a'>(PCS)</td><td bgcolor='#fbfd8a'><b>MINUS PAPER</b></td>";
								for($h=2; $h<mysqli_num_fields($result); $h++)
								{ 
									if($ju[$h]==NULL){
											$minus[$h]="";
									}else{
											$minus[$h]=round(($totfor[$h]-$warehouse[$h]-$outstanding[$h]),3);
									}
									echo "<td align=right bgcolor='#fbfd8a'><b>".$minus[$h]."</b></td>";
								} 
							 echo"</tr>";
							 
							 //REAM
							 echo "
							 <tr><td bgcolor='#fbfd8a'>(REAM)</td><td bgcolor='#fbfd8a'></td>";
								for($h=2; $h<mysqli_num_fields($result); $h++)
								{ 
									if($ju[$h]==NULL){
											$minus[$h]="";
									}else{
											$minus[$h]=round(((($totfor[$h]-$warehouse[$h]-$outstanding[$h])/$ju[$h])/500),3);
									}
									echo "<td align=right bgcolor='#fbfd8a'><b>".$minus[$h]."</b></td>";
								} 
							 echo"</tr>";
							 ...
							 //TON
							 echo "
							 <tr><td bgcolor='#fbfd8a'>(TON)</td><td bgcolor='#fbfd8a'></td>";
								for($h=2; $h<mysqli_num_fields($result); $h++)
								{ 
									if($ju[$h]==NULL){
											$minus[$h]="";
									}else{
											$minus[$h]=round(0.27*0.64*1.01*500*(((($totfor[$h]-$warehouse[$h]-$outstanding[$h])/$ju[$h])/500)/1000),3);
									}
									echo "<td align=right bgcolor='#fbfd8a'><b>".$minus[$h]."</b></td>";
								} 
							 echo"</tr>";
